...
int LogStore::write(const void *buf, size_t len, off64_t offset, int group) {
  // check against the invalidation
  alignas(512) char tmp[4096];

  buseOperations::write(buf, len, offset, group);
  ...
  for ( ;; i += 4096) {
    ...
    mManager->Append(((const char *)buf) + i, addr, group);
  }
  ...
  if (offset + len < end) {
    ...
    mManager->Append(tmp, end - 4096, group);
  } else {
    mManager->Append(((const char *)buf) + i, offset + i, group);
  }
...
LogStore::LogStore(uint64_t size, uint32_t NumGroup) :
  buse::buseOperations(size)
{
  mManager = std::make_unique<Manager>(NumGroup);
